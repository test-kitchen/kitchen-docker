...
module Kitchen
  ...
  module Driver
    ...
    class Docker < Kitchen::Driver::SSHBase
      ...
      default_config :docker_bin,          'docker'

      def verify_dependencies
        run_command("#{config['docker_bin']} > /dev/null", :quiet => true)
        rescue
          raise UserError,
      ...
      def build_image(state)
        output = run_command("#{config[:docker_bin]} build -", :input => dockerfile)
      ...
      def container_address(state)
        ...
        output = run_command("#{config[:docker_bin]} inspect #{container_id}")
      ...
      def rm_container(state)
        container_id = state[:container_id]
        run_command("#{config[:docker_bin]} stop #{container_id}")
        run_command("#{config[:docker_bin]} rm #{container_id}")
      end

      def rm_image(state)
        image_id = state[:image_id]
        run_command("#{config[:docker_bin]} rmi #{image_id}")
      end
    end
  end
end
